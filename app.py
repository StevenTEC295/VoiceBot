...
def voice():
    ...
    resp = VoiceResponse()
    gather = Gather(input="speech", action=f"/dialogflow?voice={voice}", method="POST", timeout=2,language="es-MX")
...
def dialogflow_webhook():
    ...
    twiml = VoiceResponse()
    gather = Gather(input="speech", action="/dialogflow", method="POST", timeout=2,language="es-MX")
    gather.say(bot_reply, voice=voice)
    twiml.append(gather)
